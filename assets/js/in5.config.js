...
var shareInfo = {btns:["twitter", "facebook", "gplus", "linkedin"], align:"bottom"};
...
function initClickEvents(){
	$('#container').find('*').each(function(index,el){
		...
		if(clickArr.length) { $el.on(clickEv,function(e){$.each(clickArr,function(i,func){func.call(el,e);}); if(pd){return false;} e.stopPropagation();  }); }
	});
}
...
$(window).load(function(){
	...
	sliderSettings = {
		mode: pageMode, theme: 'in5',
		buildArrows:arrowNav, buildNavigation:false,
		resizeContents: false, buildStartStop:false,
		hashTags:false, infiniteSlides:false,
		stopAtEnd:true, resumeOnVisible:false,
			...
			nav = {
				...
				to:function(n,c){
					if(c){
						...
						slider.scrollAdjust = (pageMode=='h' || pageMode=='fade') ? function(){ $(document).scrollTop(offset.top+c[1]);} : function(){$(document).scrollLeft(offset.left+c[0]);};
					} else slider.scrollAdjust = undefined;
					slider.gotoPage(n);
					if(n == slider.currentPage){
						slider.scrollAdjust();
